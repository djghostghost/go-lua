...
func (r *reader) readPrototype(parentSource string) *Prototype {
	source := r.readString()
	if source == "" {
		source = parentSource
	}
	return &Prototype{
		...
		SubPrototypes:   r.readSubPrototypes(source),
		LineInfo:        r.readLineInfo(),
		LocalVars:       r.readLocVars(),
...
func (r *reader) readSubPrototypes(source string) []*Prototype {
	prototypes := make([]*Prototype, r.readUint32())
	for i := range prototypes {
		prototypes[i] = r.readPrototype(source)
	}
	return prototypes
}
...
// 读取局部变量
func (r *reader) readLocVars() []LocalVar {
